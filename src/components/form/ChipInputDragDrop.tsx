...
interface ChipInputDragDropProps {
  name: string;
  label?: string;
  placeholder?: string;
  value: string[];
...
export const ChipInputDragDrop = ({
  name,
  label,
  ...
  const handleDragEnd = (result: DropResult) => {
    if (!result.destination || disabled || !allowDragDrop) return;
      ...
      <Droppable droppableId="chips" direction="horizontal">
        {(provided) => (
          <div
            ...
            className="flex flex-wrap gap-1 mb-2 min-h-[28px]"
          >
            {value.map((chip, index) => (
              <Draggable key={`${chip}-${index}`} draggableId={`chip-${index}`} index={index}>
                {(provided, snapshot) => (
                  <div
                    ref={provided.innerRef}
  ...
  return (
    <div className={cn("space-y-2", className)}>
      ...
      <div className={cn(
        ...
        {allowDragDrop && !disabled ? <DraggableChipList /> : <ChipList />}
