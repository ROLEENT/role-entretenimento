-- CORREÇÃO CRÍTICA DE SEGURANÇA: Habilitar RLS em tabelas (excluindo views)

-- 1. Habilitar RLS nas tabelas sem proteção (excluindo analytics_summary que é view)
...
ALTER TABLE public.cities_other_counts ENABLE ROW LEVEL SECURITY;
...
-- Cities Other Counts - Público pode ver
CREATE POLICY "Public can view cities counts" ON public.cities_other_counts
