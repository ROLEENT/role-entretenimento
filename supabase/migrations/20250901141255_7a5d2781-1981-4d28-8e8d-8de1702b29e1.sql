-- ETAPA 1: MIGRAÇÃO CRÍTICA DE DADOS
-- Migrar dados de entity_profiles para profiles
INSERT INTO profiles (
  id, handle, name, type, city, state, country, 
...
-- Atualizar perfis existentes com informações mais ricas
UPDATE profiles SET
  bio = CASE handle
...
-- Atualizar dados específicos de artistas
UPDATE profile_artist SET
  genres = CASE 
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'joaomusico') 
    THEN ARRAY['Rock Alternativo', 'MPB', 'Rock Nacional', 'Indie Rock']
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'mariapop')
    THEN ARRAY['Pop Brasileiro', 'Pop Indie', 'Electropop', 'R&B']
    ELSE genres
  END,
  instruments = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'joaomusico')
    THEN ARRAY['Guitarra', 'Violão', 'Baixo', 'Harmônica']
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'mariapop')
    THEN ARRAY['Vocal', 'Piano', 'Teclado', 'Violão']
    ELSE instruments
  END,
  experience_years = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'joaomusico') THEN 15
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'mariapop') THEN 8
    ELSE experience_years
  END,
  available_for_hire = true,
  career_highlights = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'joaomusico')
    THEN 'Participação no Festival de Inverno 2023, abertura para Skank em 2022, 3 álbuns independentes lançados'
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'mariapop')
    THEN 'Finalista do Prêmio Multishow 2023, mais de 1M de streams no Spotify, participação no The Voice Brasil'
    ELSE career_highlights
  END,
  social_media_followers = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'joaomusico') THEN 25000
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'mariapop') THEN 120000
    ELSE social_media_followers
  END
WHERE profile_id IN (
  SELECT id FROM profiles WHERE handle IN ('joaomusico', 'mariapop')
);

-- Atualizar dados específicos de locais
UPDATE profile_venue SET
  capacity = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'cultbar') THEN 200
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'studioz') THEN 80
    ELSE capacity
  END,
  venue_type = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'cultbar') THEN 'Bar/Casa de Shows'
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'studioz') THEN 'Estúdio/Sala Multiuso'
    ELSE venue_type
  END,
  amenities = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'cultbar')
    THEN ARRAY['Bar completo', 'Palco equipado', 'Sistema de som', 'Iluminação', 'Estacionamento', 'Área fumantes']
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'studioz')
    THEN ARRAY['Mesa de som profissional', 'Isolamento acústico', 'Instrumentos disponíveis', 'Gravação multipista', 'Ar condicionado']
    ELSE amenities
  END,
  technical_specs = CASE
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'cultbar')
    THEN 'Mesa de som Yamaha 24 canais, sistema de PA line array, iluminação LED RGB, palco 6x4m'
    WHEN profile_id = (SELECT id FROM profiles WHERE handle = 'studioz')
    THEN 'Pro Tools HDX, monitores Genelec, pré-amps Neve, isolation booths, Steinway piano'
    ELSE technical_specs
  END
WHERE profile_id IN (
  SELECT id FROM profiles WHERE handle IN ('cultbar', 'studioz')
);

-- Criar seguidores de exemplo
INSERT INTO followers (follower_id, following_id, created_at)
SELECT 
  user_1.id as follower_id,
  user_2.id as following_id,
  now() - interval '30 days' * random() as created_at
FROM (
  SELECT id FROM profiles WHERE handle IN ('joaomusico', 'mariapop', 'cultbar') LIMIT 3
) user_1
CROSS JOIN (
  SELECT id FROM profiles WHERE handle IN ('joaomusico', 'mariapop', 'cultbar', 'studioz', 'eventospro') LIMIT 5
) user_2
WHERE user_1.id != user_2.id
AND NOT EXISTS (
  SELECT 1 FROM followers 
  WHERE follower_id = user_1.id AND following_id = user_2.id
)
LIMIT 8;
