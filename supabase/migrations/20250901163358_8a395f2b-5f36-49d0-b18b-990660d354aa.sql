-- Create triggers to synchronize administrative forms with entity_profiles
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM entity_profiles WHERE source_id = NEW.id AND type = 'organizador') THEN
        ...
        UPDATE entity_profiles SET
            ...
            instagram = NEW.instagram,
            website = NEW.site,
            links = jsonb_build_object('website', NEW.site),
            updated_at = NOW()
    ...
    ELSE
        ...
        INSERT INTO entity_profiles (
            ...
            NEW.instagram,
            NEW.site,
            jsonb_build_object('website', NEW.site),
            CASE WHEN NEW.status = 'active' THEN 'published' ELSE 'draft' END,
            NOW(),
            NOW()
        );
...
-- Function to sync venue data to entity_profiles
...
BEGIN
    ...
    IF EXISTS (SELECT 1 FROM entity_profiles WHERE source_id = NEW.id AND type = 'local') THEN
        ...
        UPDATE entity_profiles SET
            ...
            instagram = NEW.instagram,
            website = NEW.website,
            links = jsonb_build_object('website', NEW.website),
    ...
    ELSE
        ...
        INSERT INTO entity_profiles (
            ...
            NEW.instagram,
            NEW.website,
            jsonb_build_object('website', NEW.website),
            NEW.cover_url,
            NEW.tags,
            CASE WHEN NEW.status = 'active' THEN 'published' ELSE 'draft' END,
            NOW(),
...
INSERT INTO entity_profiles (
    source_id, type, name, handle, bio, bio_short, city, country,
    contact_email, contact_phone, instagram, website, avatar_url, cover_url,
    tags, links, status, created_at, updated_at
)
SELECT 
    a.id,
    'artista',
...
INSERT INTO entity_profiles (
    source_id, type, name, handle, bio, bio_short, city, country,
    contact_email, contact_phone, instagram, website, links, status, created_at, updated_at
)
SELECT 
    o.id,
    'organizador',
    ...
    o.instagram,
    o.site,
    jsonb_build_object('website', o.site),
...
INSERT INTO entity_profiles (
    source_id, type, name, handle, bio, bio_short, city, country,
    contact_email, contact_phone, instagram, website, links, cover_url,
...
SELECT 
    ...
    v.instagram,
    v.website,
    jsonb_build_object('website', v.website),
    v.cover_url,
    v.tags,
