-- Fix duplicate slug issue and complete the migration
...
-- Handle duplicate/empty slugs by making them unique
UPDATE public.genres 
SET slug = CASE 
  WHEN name IS NULL OR trim(name) = '' THEN 'unnamed-genre-' || id::text
  ELSE public.to_slug(name) || '-' || substring(id::text from 1 for 8)
END 
WHERE slug IS NULL OR slug = '' OR slug IN (
  SELECT slug FROM public.genres 
...
BEGIN
  ...
  IF NEW.slug IS NULL OR NEW.slug = '' OR (OLD.name IS DISTINCT FROM NEW.name) THEN
    ...
    WHILE EXISTS (SELECT 1 FROM public.genres WHERE slug = NEW.slug AND id != NEW.id) LOOP
