...
CREATE TABLE IF NOT EXISTS public.event_series_items (
  ...
  event_id  uuid NOT NULL, -- FK criada após a tabela events
  edition_number int,
...
CREATE TABLE IF NOT EXISTS public.events (
  ...
  doors_open_utc   timestamptz,
  headliner_starts_utc timestamptz,
  summary          text,
  description      text NOT NULL,
  min_price        numeric(10,2),
  max_price        numeric(10,2),
  age_rating       public.age_rating DEFAULT 'L',
  age_notes        text,
  highlight_type   public.highlight_type NOT NULL DEFAULT 'none',
...
-- completar FK de event_series_items -> events
ALTER TABLE public.event_series_items
  ADD CONSTRAINT fk_esi_event FOREIGN KEY (event_id) REFERENCES public.events(id) ON DELETE CASCADE;

CREATE INDEX IF NOT EXISTS idx_events_start ON public.events(start_utc);
...
CREATE OR REPLACE FUNCTION public.fn_events_before_ins_upd()
RETURNS trigger AS $$
BEGIN
  IF NEW.slug IS NULL OR NEW.slug = '' THEN NEW.slug := public.fn_slugify(NEW.title); END IF;
  IF NEW.status = 'published' AND NEW.published_at IS NULL THEN NEW.published_at := now(); END IF;
  NEW.updated_at := now();
  IF NEW.highlight_type = 'vitrine' THEN NEW.is_sponsored := true; END IF;
  RETURN NEW; END; $$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_events_before ON public.events;
CREATE TRIGGER trg_events_before
BEFORE INSERT OR UPDATE ON public.events
FOR EACH ROW EXECUTE FUNCTION public.fn_events_before_ins_upd();
...
-- Políticas exemplo: permitir INSERT anônimo como rascunho; leitura pública só de publicados
CREATE POLICY events_insert_public ON public.events FOR INSERT TO anon
...
DECLARE v_event_id uuid; v_series_id uuid; v_slot_id uuid; i int; BEGIN
  ...
  INSERT INTO public.events (
    ...
    cover_url, cover_alt, start_utc, end_utc, doors_open_utc, headliner_starts_utc,
    summary, description, min_price, max_price, age_rating, age_notes,
    highlight_type, is_sponsored, ticketing, ticket_rules, links, accessibility, gallery,
    ...
    COALESCE(p->>'age_rating','L')::public.age_rating, NULLIF(p->>'age_notes',''),
    COALESCE(p->>'highlight_type','none')::public.highlight_type, COALESCE(p->>'is_sponsored','false')::boolean,
    p->'ticketing', p->'ticket_rules', p->'links', p->'accessibility', p->'gallery',
    COALESCE(ARRAY(SELECT jsonb_array_elements_text(p->'tags')), '{}'),
    COALESCE(ARRAY(SELECT jsonb_array_elements_text(p->'genres')), '{}'),
    COALESCE(p->>'status','draft')::public.publication_status, NULLIF(p->>'publish_at','')::timestamptz,
  ...
  IF p ? 'series' THEN
    ...
    IF v_series_id IS NOT NULL THEN
      ...
      UPDATE public.events SET series_id = v_series_id WHERE id = v_event_id;
    END IF;
  END IF;
