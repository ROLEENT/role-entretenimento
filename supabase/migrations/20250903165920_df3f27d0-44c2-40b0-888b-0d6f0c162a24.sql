-- Fase 2: Estrutura Completa do Banco de Dados (Syntax Fix)

-- 1. Criar enums em falta (sem IF NOT EXISTS para enums)
DO $$ 
BEGIN
...
-- Adicionar foreign key para series_id se a coluna foi criada
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.table_constraints 
    WHERE constraint_name = 'events_series_id_fkey'
  ) THEN
    ALTER TABLE public.events 
