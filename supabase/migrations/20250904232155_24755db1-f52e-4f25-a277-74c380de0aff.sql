...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'criterion_key') THEN
    CREATE TYPE criterion_key AS ENUM (
      'relevancia','qualidade','diversidade','impacto',
      'coerencia','experiencia','tecnica','acessibilidade'
    );
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'criterion_status') THEN
    CREATE TYPE criterion_status AS ENUM ('met','partial','na');
  END IF;
END $$;
