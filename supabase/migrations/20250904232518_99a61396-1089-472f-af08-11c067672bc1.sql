...
-- Primeiro, inserir um evento com dados básicos
...
-- Buscar o ID do evento para usar nas próximas inserções
DO $$
DECLARE
  event_uuid UUID;
BEGIN
  ...
  -- Inserir critérios de curadoria para o evento
  ...
  -- Atualizar as notas de curadoria
  ...
  -- Recalcular o score
  PERFORM public.recalc_event_curation_score(event_uuid);
END $$;

-- Criar também um exemplo de vitrine cultural (evento patrocinado)
INSERT INTO public.events (
  id,
  title,
  slug,
