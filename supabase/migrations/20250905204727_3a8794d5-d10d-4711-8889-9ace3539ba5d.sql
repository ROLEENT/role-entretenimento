-- 1. Criar nova função de validação de admin baseada em approved_admins
CREATE OR REPLACE FUNCTION public.is_admin_session_valid_v2(session_email text)
RETURNS boolean
LANGUAGE sql
...
-- 2. Atualizar função soft_delete_event para usar approved_admins
...
BEGIN
  ...
  -- Verificar se é admin válido usando approved_admins
  IF NOT EXISTS (
    SELECT 1 FROM public.approved_admins 
...
-- 3. Criar função para deletar evento via admin com validação completa
