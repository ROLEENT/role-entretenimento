-- Criar tabelas para funcionalidades de rolezeiros (sem mexer na tabela follows existente)
...
DROP POLICY IF EXISTS "Public can view public attendance" ON public.attendance;
...
DROP POLICY IF EXISTS "Users can manage their own profile" ON public.users_public;
CREATE POLICY "Users can manage their own profile" ON public.users_public
FOR ALL USING (auth.uid() = id)
WITH CHECK (auth.uid() = id);

DROP POLICY IF EXISTS "Public can view public profiles" ON public.users_public;
...
DROP TRIGGER IF EXISTS update_users_public_updated_at ON public.users_public;
