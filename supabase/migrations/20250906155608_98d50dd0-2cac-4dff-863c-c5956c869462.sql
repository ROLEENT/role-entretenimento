-- RECUPERAÇÃO CORRIGIDA DOS ARTISTAS ORIGINAIS
-- 1. Remover função de validação defeituosa (já foi removida)

-- 2. Restaurar artistas originais com UUIDs válidos
...
-- PV5000 - Artista principal de techno/house (RECUPERADO)
(
    'PV5000',
    'DJ/Producer',
    'São Paulo',
...
-- Gau Beats - Produtor brasileiro (RECUPERADO)
(
    'Gau Beats',
    'Producer',
    'Rio de Janeiro',
...
-- AKILA - Artista de música urbana (RECUPERADO)
(
    'AKILA',
    'Singer/Songwriter',
    'Salvador',
...
-- Linguini - DJ/Producer (RECUPERADO)
(
    'Linguini',
    'DJ/Producer',
    'Belo Horizonte',
...
-- Hate Moss - Artista experimental (RECUPERADO)
(
    'Hate Moss',
    'Electronic Artist',
    'Porto Alegre',
...
-- Remover artistas fake criados anteriormente
DELETE FROM public.artists 
WHERE stage_name IN ('DJ Silva', 'Banda Eletrônica', 'MC Freestyle', 'Duo Acústico', 'Produtora Musical X');

-- 3. Criar função de validação corrigida (sem bloquear PV5000)
CREATE OR REPLACE FUNCTION public.validate_artist_name_safe()
RETURNS trigger
LANGUAGE plpgsql
AS $function$
BEGIN
  -- Verificar apenas nomes CLARAMENTE fake/teste (PV5000 não está mais na lista)
...
-- Aplicar validação segura (se não existir)
DROP TRIGGER IF EXISTS validate_artist_name_safe ON public.artists;
...
-- Verificar que os artistas foram restaurados
SELECT 
    stage_name,
    city,
    status,
    created_at
FROM public.artists 
WHERE stage_name IN ('PV5000', 'Gau Beats', 'AKILA', 'Linguini', 'Hate Moss')
ORDER BY stage_name;