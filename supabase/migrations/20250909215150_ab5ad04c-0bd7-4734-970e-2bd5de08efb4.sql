-- Fix create_event_cascade function to include organizer_id
CREATE OR REPLACE FUNCTION public.create_event_cascade(event_data jsonb, partners jsonb DEFAULT '[]'::jsonb, lineup_slots jsonb DEFAULT '[]'::jsonb, performances jsonb DEFAULT '[]'::jsonb, visual_artists jsonb DEFAULT '[]'::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  visual_item jsonb;
BEGIN
  -- 1. Criar evento principal (incluindo organizer_id e curatorial_criteria)
  INSERT INTO public.events (
    title, subtitle, summary, description,
    venue_id, organizer_id, location_name, address, city, state, country,
