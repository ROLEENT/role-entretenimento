-- Fase 2: Correção de Relacionamentos Órfãos (Corrigido)
...
-- 4. Criar entity_profiles para venues que não têm (usando apenas colunas existentes)
INSERT INTO entity_profiles (
  source_id, type, name, handle, bio, bio_short, 
  city, state, country, contact_email, contact_phone,
  avatar_url, links, visibility, created_at, updated_at
)
SELECT DISTINCT
  v.id,
  'local',
  ...
  v.email,
  v.phone,
  v.logo_url, -- usando logo_url como avatar_url
  jsonb_build_object(
